...
import { addMeal, addMeals, getMealsByArea, getMealById, getMealsByName } from "../services/dbService";

export async function useMealByName(name:string): Promise<mealPreview[]>{
...
export async function useMealByCategory(category:string): Promise<mealPreview[]>{
    const dbResult = getMealsByName(category);
    if (dbResult.length === 0) {
...
export async function useMealByArea(area:string): Promise<mealPreview[]>{
